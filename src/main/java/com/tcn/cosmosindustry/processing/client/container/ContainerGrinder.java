...
import com.tcn.cosmosindustry.processing.client.container.slot.SlotGrinder;
...
public class ContainerGrinder extends CosmosContainerMenuBlockEntity {
	
	public ContainerGrinder(int indexIn, Inventory playerInventoryIn, FriendlyByteBuf extraData) {
		this(indexIn, playerInventoryIn, new SimpleContainer(6), ContainerLevelAccess.NULL, extraData.readBlockPos());
	}

	public ContainerGrinder(int indexIn, Inventory playerInventoryIn, Container tile, ContainerLevelAccess accessIn, BlockPos posIn) {
		super(IndustryModBusManager.CONTAINER_TYPE_GRINDER, indexIn, playerInventoryIn, accessIn, posIn);
		
		//Input Slot
		this.addSlot(new Slot(tile, 0, 104, 18));
		
		//Output Slot
		this.addSlot(new SlotGrinder(player, tile, 1, 104, 58));

		//Secondary Output Slot
		this.addSlot(new SlotGrinder(player, tile, 2, 147, 50));
	...
	public boolean stillValid(Player playerIn) {
		return stillValid(this.access, playerIn, IndustryModBusManager.BLOCK_GRINDER);
	}
	...
	public ItemStack quickMoveStack(Player playerIn, int indexIn) {
		...
		if (flag && slot != null && slot.hasItem()) {
			...
			if (indexIn >= 0 && indexIn < 6 ) {
				...
				else if (this.getLevel().getRecipeManager().getRecipeFor(IndustryCraftingManager.RECIPE_TYPE_GRINDING, new SimpleContainer(itemstack), getLevel()).isPresent()) {
