...
public class SlotOrePlant extends Slot {
	...
	public SlotOrePlant(Player entity_player, Container inventory, int index, int x_pos, int y_pos) {
	...
	protected void checkTakeAchievements(ItemStack stack) {
		...
		//if (this.container instanceof TileEntityOrePlant) {
			//TileEntityOrePlant tileEntity = container;
			
			if (!this.player.level.isClientSide) {
				int i = this.amount;
				float f = OrePlantCleaningRecipes.getInstance().getRecipeExperienceValue(stack);
