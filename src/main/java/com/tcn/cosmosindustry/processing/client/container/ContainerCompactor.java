...
import com.tcn.cosmosindustry.processing.client.container.slot.SlotCompactor;
...
public class ContainerCompactor extends CosmosContainerMenuBlockEntity {
	
	public ContainerCompactor(int indexIn, Inventory playerInventoryIn, FriendlyByteBuf extraData) {
		this(indexIn, playerInventoryIn, new SimpleContainer(6), ContainerLevelAccess.NULL, extraData.readBlockPos());
	}
	
	public ContainerCompactor(int indexIn, Inventory playerInventoryIn, Container contentsIn, ContainerLevelAccess accessIn, BlockPos posIn) {
		super(IndustryModBusManager.CONTAINER_TYPE_COMPACTOR, indexIn, playerInventoryIn, accessIn, posIn);
		
		/**@Inputslot*/
		this.addSlot(new Slot(contentsIn, 0, 104, 18));

		/**@Powerslot*/
		//this.addSlotToContainer(new SlotBattery(tile, 1, 77, 61));

		/**@OutputSlot*/
		this.addSlot(new SlotCompactor(player, contentsIn, 2, 104, 58));

		/**@Upgradeslots*/
		this.addSlot(new SlotUpgrade(contentsIn, 3, 56, 18, IndustryModBusManager.UPGRADE_SPEED));
		this.addSlot(new SlotUpgrade(contentsIn, 4, 56, 38, IndustryModBusManager.UPGRADE_CAPACITY));
		this.addSlot(new SlotUpgrade(contentsIn, 5, 56, 58, IndustryModBusManager.UPGRADE_EFFICIENCY));
	...
	public boolean stillValid(Player playerIn) {
		return stillValid(this.access, playerIn, IndustryModBusManager.BLOCK_COMPACTOR);
	}
