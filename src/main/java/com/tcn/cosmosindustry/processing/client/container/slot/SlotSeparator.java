...
public class SlotSeparator extends Slot {
	
	private Player player;
	private int amount;

	public SlotSeparator(Player entity_player, Container inventory, int index, int x_pos, int y_pos) {
	...
	protected void checkTakeAchievements(ItemStack stack) {
		...
		if (!this.player.level.isClientSide) {
			int i = this.amount;
			float f = SeparatorRecipes.getInstance().getRecipeExperienceValue(stack);
